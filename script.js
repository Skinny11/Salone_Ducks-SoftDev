...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('🖼️ Hero slideshow initialized with', slides.length, 'slides');
});
...
async function startCamera() {
    try {
        ...
        if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
            ...
            errorMsg += 'No camera found on your device. Please connect a camera and try again.';
        } else if (err.name === 'NotReadableError') {
            errorMsg += 'Your camera is in use by another application. Please close other apps and try again.';
        } else {
...
function capturePhoto() {
    if (!video.srcObject || !cameraStream || !cameraStream.active) {
...
function stopCamera() {
    if (cameraStream) {
        ...
        // Show placeholder if no photo captured
        if (!capturedImageDataUrl && cameraPlaceholder) {
        ...
        console.log('ℹ️ No active camera to stop');
    }
}
...
document.addEventListener('keydown', function(event) {
    ...
    // Press 's' to stop camera
    if (event.key === 's' || event.key === 'S') {
        if (stopCameraBtn.style.display !== 'none' && !stopCameraBtn.disabled) {
            stopCamera();
        }
    }
    // Press 'r' to retake
    if (event.key === 'r' || event.key === 'R') {
        if (resetCameraBtn.style.display !== 'none') {
            resetCameraView();
        }
    }
});

console.log('📷 Camera module loaded. Shortcuts: [C] Capture, [S] Stop, [R] Retake');
